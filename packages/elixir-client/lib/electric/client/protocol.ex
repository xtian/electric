defmodule Electric.Client.Protocol do
  @moduledoc false
  alias Electric.Client
  alias Electric.Client.{ExpiredShapesCache, Fetch, Message, ShapeKey, ShapeState, TagTracker}
  @max_stale_retries 3
  def request(client, state, opts) do
    ...
    handle_response(Fetch.request(client, request), client, state, shape_key)
  end

  def handle_response(result, client, state, shape_key) do
    case result do
  ...
  defp handle_success(resp, client, state, shape_key) do
    case validate_handle(resp, state, shape_key) do
      :ok -> process_success_response(resp, client, state, resp.shape_handle)
      result -> result
    end
  end

  def validate_handle(resp, state, shape_key) do
    if resp.shape_handle == ExpiredShapesCache.get_expired_handle(shape_key) do
  ...
  def validate_headers!(%Fetch.Response{} = resp, %ShapeState{} = state) do
